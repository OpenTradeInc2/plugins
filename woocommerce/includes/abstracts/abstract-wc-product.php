...
class WC_Product {
	 ...
	 * @return string
	 */
	public function get_price_with_measure_html( $price = '' ) {
		...
		if($packaging_unit == null or $packaging_unit == ''){
			...
			foreach ($this->get_attributes() as $atr){
				if($atr[name] == 'Packaging Measure'){
					$packaging_measure = $atr[value];
				}
				if($atr[name] == 'Packaging Unit'){
					$packaging_unit = $atr[value];
				}
			}
		}
		...
		if($packaging_measure == null or $packaging_measure == ''){
			...
			foreach ($this->get_attributes() as $atr){
				...
				if($atr[name] == 'Price / lb'){
					$price_by_lb = $atr[value];
				}
				if($atr[name] == 'Price / Kg'){
					$price_by_kg = $atr[value];
				}
			}
		}

		if(strtolower($packaging_measure) == 'kg'){
			$price = '<span class="amount">&#36;'.$price_by_kg.'/kg</span>';
		}else{
			$price = '<span class="amount">&#36;'.$price_by_lb.'/lb</span>';
		}
