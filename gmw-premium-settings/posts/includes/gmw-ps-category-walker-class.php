 ...
 */
class GMW_PT_Categories_Walker extends Walker {
	...
	public function start_el( &$output, $term, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if ( 'checkbox' == $args['style'] ) {
			...
			$checkbox  = '<li class="gmw-checkbox-wrapper checkbox-id-'.$term->term_id.'-wrapper '.$category_icon_class.'">'; 
			$checkbox .= '<input type="checkbox" name="tax_'.esc_attr( $args['taxonomy'] ).'[]" id="gmw-checkbox-id-'.$term->term_id.'" class="gmw-single-checkbox gmw-checkbox-'.esc_attr( $args['taxonomy'] ).' gmw-checkbox-id-'.$term->term_id.'" value="'.$term->term_id.'" '.$checked.' '.$display_none.' />';
			
			if ( $category_icon_ok ) {
				$checkbox .= '<img id="gmw-cat-icon-'.$term->term_id.'" class="category-icon gmw-checkbox-cat-icons '.$icon_checked.'" src="'.esc_url( $args['cat_icons']['url'].$args['cat_icons']['icons'][$term->term_id] ).'" onclick="jQuery(this).toggleClass(\'checked\');jQuery(this).closest(\'li.gmw-checkbox-wrapper\').find(\'.gmw-single-checkbox\').click();" />';
			}
			
			$checkbox .= "<label for=\"gmw-checkbox-id-$term->term_id\">$term_name</label>";
			$checkbox .= '</li>';
			...
			$output   .= "\t<option class=\"level-$depth\" value=\"$term->term_id\" $selected>$pad $term_name</option>\n";	
		}
	}
}
