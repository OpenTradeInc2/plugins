<?php 
/**
 * Posts Locator "left-white" pop-up window tempalte file . 
 * 
 ...
 * Instead you can copy-paste this template ( the "left-white" folder contains this file and the "css" folder ) 
 ...
 * It will show in the pop-up tempaltes dropdown menu as "Custom: left-white".
 */
?>
...
<div class="top-buttons-wrapper">		
	...
	<?php gmw_window_toggle( $gmw, $post, false, 'gmw-iw-template-holder', 'dashicons-arrow-right-alt2', 'dashicons-arrow-left-alt2', 'width', '30px', '100%' ); ?>
	
</div>

<?php do_action( 'gmw_pt_before_template', $post, $gmw ); ?>
...
<div class="template-content-wrapper">
	...
	<?php if ( isset( $gmw['info_window']['draggable_use'] ) ) { ?>
		<?php echo gmw_get_draggable_handle( $gmw, false, 'gmw-iw-template-holder', true, 'dashicons-editor-justify', 'window' ); ?>		
	<?php } ?>
	
	<!-- featured image -->	
	<?php if ( isset( $gmw['info_window']['featured_image'] ) && has_post_thumbnail( $post->ID ) ) { ?>  	
    	
    	<?php do_action( 'gmw_pt_template_before_image', $post, $gmw ); ?>
	...
	<?php do_action( 'gmw_pt_template_before_title', $post, $gmw ); ?>
	...
	<?php if ( !empty( $gmw['info_window']['additional_info']['formatted_address'] ) ) { ?>
		...
		<?php do_action( 'gmw_pt_template_before_address', $post, $gmw ); ?>
    ...
    <?php if ( isset( $gmw['info_window']['live_directions'] ) ) { ?>   
	    ...
	    <?php do_action( 'gmw_pt_template_before_live_directions', $post, $gmw ); ?>
	    	    
	   	<?php gmw_live_directions( $post, $gmw , '' ) ?>
	   	 
	   	<?php do_action( 'gmw_pt_template_before_directions_panel', $post, $gmw ); ?>
